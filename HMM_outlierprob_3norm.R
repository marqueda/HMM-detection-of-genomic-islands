...
# - filename_3state_HMMparest.txt : Text file with Baum-Welch algorithm optimized parameters from nstart [default=1000] random starting values.
# - filename_3state_parest_plot.pdf : Scatterplot of the maximized parameter estimates against their likelihood
# - filename_zscore_vs_3statedistributions.pdf : Histogram of the z-transformed q-values with the distribution and parameters of the three states.
# - filename_3state_bestparameters.txt : Text file with the best HMM parameters used for the state reconstruction
# - filename_3state_HMMstates.txt : Text file with the HMM states from the state reconstruction using the best parameters
# - filename_3state_PLISsignificantsites.txt : Text file with multiple testing corrected significant sites coded as 0 and 1 (significant SNPs after multiple testing)
...
write.table(hmmpar,file=paste(base,"_3state_HMMparest.txt",sep=""))
hmmpar<-read.table(paste(base,"_3state_HMMparest.txt",sep=""),stringsAsFactors=F)
...
write.table(bestpar,file=paste(base,"_3state_bestparameters.txt",sep=""))

# Plots the data (z-scores) against the inferred state distributions
pdf(file=paste(base,"_zscore_vs_3statedistributions.pdf",sep=""))
par(mfrow=c(1,1))
...
write.table(plis_res$States,file=paste(base,"_3state_PLISsignificantsites.txt",sep=""))
